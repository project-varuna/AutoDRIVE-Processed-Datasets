...
This repository processes and compiles all individual files from the offroad [`AutoDRIVE-Hunter-SE-Dataset`](https://github.com/Tinker-Twins/AutoDRIVE-Hunter-SE-Dataset/tree/on-road-dataset) into the "AutoDRIVE" format used in the Project Varuna GUI. The original dataset was collected using the AutoDRIVE Ecosystem, capturing data from the **Hunter SE** vehicle. For more information, visit the [AutoDRIVE-Hunter-SE repository](https://github.com/Tinker-Twins/AutoDRIVE).

## Dataset Structure

The following structure should be used when working with the "Project-Varuna-Custom" format in the GUI application:

| **DATA** | timestamp | throttle |	steering | leftTicks | rightTicks |	posX | posY |	posZ | roll |	pitch |	yaw |	speed |	angX |	angY |	angZ | accX |	accY | accZ | command_velocity |
| -------- | --------- | -------- |	-------- | --------- | ---------- |	---- | ---- |	---- | ---- |	----- |	--- |	----- |	---- |	---- |	---- | ---- |	---- | ---- | ----  |
| **UNIT** | yyyy_MM_dd_HH_mm_ss_fff | norm% | rad | count | count |	m | m |	m | rad |	rad |	rad |	m/s |	rad/s |	rad/s |	rad/s | m/s^2 |	m/s^2 | m/s^2 | m/s |

Ensure any custom datasets follow this structure if your choosing this option in the GUI.
...
- **maneuver**: Type of maneuver for data-collection (Skidpad,Fishhook,Slalom etc.)
- **frequency**: Indicates the frequency at which data is collected.
- **orientation**: Indicates whether the run was conducted in clockwise/counter-clockwise direction.
